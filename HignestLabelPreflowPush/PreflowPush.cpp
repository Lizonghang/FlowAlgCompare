...
// 获取activeList中最大d值的节点ID,并将该点从activeList中移除
int CGraph::getHignestLabelVertexID()
{
    list<int>::iterator l, tempIter;
    int mind = 0;
    int tempVertexID;
    for (l = activeList.begin(); l != activeList.end(); l++)
    {
        if (listVertex[*l]->d > mind)
        {
            mind = listVertex[*l]->d;
            tempIter = l;
        }
    }
    tempVertexID = *tempIter;
    activeList.erase(tempIter);
    return tempVertexID;
}

// 预流推送算法
void CGraph::HignestLabelPreflowPush()
{
    ...
    {
        // Hignest-Label Preflow-Push 算法
        minh = INT_MAX;     // 不存在admissible边时的min{d(j)+1}记录
        tempActiveVertexID = getHignestLabelVertexID();
        existAdmissible = 0;
